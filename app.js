...
function buildChart() {
  ...
  // Catatan: skala harga memakai autoscale BAWAAN candle series (pasti benar,
  // candle selalu tampil). Semua overlay (EMA/Bollinger/level) memakai
  // autoscaleInfoProvider:()=>null supaya TIDAK menggeser skala.
...
function addPriceLine(price, color, title, style = LightweightCharts.LineStyle.Dashed) {
  // Digambar sebagai LINE SERIES horizontal (bukan price line) supaya
  // TIDAK ikut auto-scale — ini kunci agar candle tidak berubah ukuran/hilang.
  if (price == null || !isFinite(price)) return null;
  const candles = S.candles;
  if (candles.length < 2) return null;
  const t0 = Math.floor(candles[0].t / 1000);
  const t1 = Math.floor(candles[candles.length - 1].t / 1000);
  const s = chart.addLineSeries({
...
async function init() {
  ...
  setInterval(loadTickers, 5000);    // harga live tiap 5 detik (menggerakkan candle terakhir)
